...
public class MainActivity extends Activity {
	...
	private final class ControlEventListener implements ControlV1.EventListener {
		...
		public ControlEventListener() {
			...
			// refresh the dialog based on the connection state
			ControlV1.State state = ControlV1.getInstance().getState();
			refresh(state);
		}

		private void refresh(final ControlV1.State state) {
			runOnUiThread(new Runnable() {
				@Override
				public void run() {
					if (true == state.equals(ControlV1.State.CONNECTED)) {
						// hide the connection status dialog
						dialog.hide();
						// populate the level views now that we're connected
						populateLevelViews();
					} else {
